...
export async function sendMail({ to, name, subject, body }) {
  ...
  const sendOne = async (recipient) => {
    try {
      ...
      console.log("sendMail to:", recipient, "response:", sendResult && sendResult.response ? sendResult.response : sendResult);
      return sendResult;
    } catch (err) {
  ...
  // If `to` is an array send one-by-one with a small delay to avoid relay limits
  if (Array.isArray(to)) {
