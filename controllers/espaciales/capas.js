...
async descargarExcelSeleccionArea(req, res) {
  ...
  async descargarExcelFiltrosCCPP(req, res) {
    ...
    try {
      ...
      // Subtítulo (A2:B2)
      worksheet.mergeCells('A2:B2');
      worksheet.getCell('A2').value = `Cobertura de Servicios de ${tipoServicio === "S" ? "Establecimiento de Salud (EE.SS.)" : "Institución Educativa (II.EE.)"} públicos de ${tipoServicio === "S" ? 'categoría/s ' + datosResumen.selectedCategorias : 'nivel ' + datosResumen.nivelSeleccionado} dentro de ${datosResumen.labelDistanciaSeleccionado}`;
